...
- **市场价格监控**:
  - 实时追踪主流 DeFi 代币 (ETH, AAVE, UNI, WBTC 等) 的预言机价格。
  - 响应式网格布局，清晰直观。
- **网络 Gas 分析**:
  - 实时监控以太坊网络拥堵状况 (最近 5 个区块)。
  - 提供最新、中位数、高位/低位均值等关键指标，辅助交易决策。
- **详细日志与调试**:
  - 提供分类日志 (Network, Oracle, Analysis)，支持过滤和一键清除。
  - 在开发者控制台中提供详细的操作和计算日志，便于调试和验证。
...
_该项目旨在提供一个清晰、透明的 DeFi 收益分析工具。_
